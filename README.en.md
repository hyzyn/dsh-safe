...
| `--to <version\|tag>` | Target dsh version (a dist-tag such as `next` works too), also how you roll back (explicit downgrades allowed); dsh-safe itself always upgrades to the latest |
...
| `DSH_SAFE_NO_UPDATE_CHECK=1` | Disable boot-time notices (the at-most-daily dsh-safe new-version notice, the dsh `next`-channel gap hint); explicit `update` / `--check` are unaffected |
...
Version channels: only npm's `latest` is followed (one `npm view <pkg> dist-tags --json` call fetches every channel, so boot cost does not change). Upstream often publishes a new version to `next` first, which produces the "latest is already current, but `next` is newer" situation — `-u` still reports "already up to date on the latest channel", **but now follows it with a one-line channel-gap hint**, and never auto-upgrades to `next` (dsh-safe does not decide for you to move onto an unreleased channel). To follow it explicitly: `dsh-safe update --to next`. On the boot path that hint is shown at most once a day; explicit calls such as `update` and `--check` always report it. To ask "would it upgrade, and to what?" without changing anything, use `dsh-safe update --check`.
...
- **First-party protection**: rows of `@deepseek-ai/*` plugins are skipped by default (disabling plugins like `dsh-web-app` would strip dsh of its core capabilities); pass `--allow-first-party` to touch them. Duplicate dedupe is guarded the same way: official sources are kept by default, and removing an official bundle requires an explicit flag or interactive confirmation, so official rows like webserver are never unmounted as collateral. The package name used for this decision falls back from the patch row to other layers with the same id, and finally to the **error text itself**, so a profile override row that omits `name` (such as webserver's host/port override) cannot silently defeat the protection.
- **Reserved entries are never auto-disabled**: `webserver` (the web UI host entry) and loader-mechanism rows (`include` / `cordis:*`) are excluded from auto-quarantine. Disabling them silently breaks the UI or the patch-composition chain, and their failures usually sit on the environment side (for example the port is already held by another dsh instance). This layer does not depend on package-name resolution, so it cannot be bypassed by a name that fails to resolve; `--allow-first-party` does not open it up — edit the patch manually if you really need to.
