...
说明:
  ...
  - 只跟随 npm 的 latest 通道。latest 之外的通道（next / alpha / beta 等）有更新时会
    提示一行（不自动安装），想跟进: dsh-safe update --to <tag>（--to 也接受 dist-tag）。
    这条提示在 -u / update / --check 上都每次报，-u <dsh 参数> 的启动路径同样每次报
    ——启动时也会打一行状态（已是最新 / 有更新）。
...
Notes:
  ...
  - The npm "latest" channel is the only one followed. When a channel other than
    latest (next / alpha / beta, …) has something newer, one hint line is printed
    (never installed automatically); to follow it: dsh-safe update --to <tag>
    (--to also accepts a dist-tag). update / --check and the -u <dsh args> boot
    path all report it every time; the boot path also prints a status line
    ("already up to date" / "updating").
