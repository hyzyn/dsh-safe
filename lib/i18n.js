...
说明:
  ...
  - 只跟随 npm 的 latest 通道。latest 已最新但 next 有更新时会提示一行（不自动安装），
    想上 next 通道: dsh-safe update --to next（--to 也接受 dist-tag）。
...
`,
  ...
  updateNextHint: '[dsh-safe] next 通道已有 {next}（当前 latest 通道 {current}）；跟进: dsh-safe update --to next',
...
Notes:
  ...
  - The npm "latest" channel is the only one followed. When latest is already
    current but "next" has something newer, one hint line is printed (never
    installed automatically); to move to next: dsh-safe update --to next
...
`,
  ...
  updateNextHint: '[dsh-safe] the next channel has {next} (latest is {current}); to follow it: dsh-safe update --to next',
