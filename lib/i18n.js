...
用法:
  ...
  dsh-safe -u [<dsh 参数…>]         先升级 dsh（已最新则跳过），再启动；不接任何参数
                                    时只升级、不启动
  ...
  dsh-safe restore --profile <名> (--id <id> | --all) [--dry-run]
                                    恢复被自动禁用的插件（升级修复后使用）
  dsh-safe update [-y] [--to <版本|tag>] [--check] [--self] [--no-restore] [--no-verify] [--pm npm|pnpm]
                                    升级 dsh 与 dsh-safe 自身，并自动恢复被隔离的插件；
                                    --check 只报告会不会升、升到什么，不安装不启动
...
包装模式选项（必须写在 profile / 子命令之前）:
  ...
  --allow-first-party      duplicate 去重时允许移除官方 bundle（隔离永不触碰核心依赖）
...
说明:
  ...
  - 环境类失败（EADDRINUSE 端口被占 / 权限 / 网络等）一律不隔离、不改任何文件，
    只透传退出码并说明原因——环境问题会让健康插件也失败，此时隔离必是误判。
  - dsh 自身依赖的核心条目（官方插件 / 官方 bundle 挂载的行 / 加载机制层 / web UI
    入口 webserver）任何情况下都不自动禁用，也没有旗标可放开；包名无法确定的行
    同样不隔离。需要处理请手动编辑 patch。
  - update/-u 会同时检查 dsh 与 dsh-safe 自身；--self 只更新 dsh-safe。
  - 只跟随 npm 的 latest 通道。latest 之外的通道（next / alpha / beta 等）有更新时，
    每个更新的通道各提示一行、按版本升序（不自动安装），想跟进: dsh-safe update
    --to <tag>（--to 也接受 dist-tag）。这条提示在 -u / update / --check 上都每次报，
    -u <dsh 参数> 的启动路径同样每次报——启动时也会打一行状态（已是最新 / 有更新）。
  - 启动时每天最多提示一次 dsh-safe 新版；DSH_SAFE_NO_UPDATE_CHECK=1 关闭启动期的
    更新状态与通道提示（显式 update / --check 不受该开关影响）。
...
`,
  ...
  skipCore:
    '[dsh-safe] 跳过核心依赖 {id}（dsh 自身依赖的插件 / 官方 bundle 挂载的行 / 加载机制层，任何情况下都不自动禁用）；失败原因: {reason}。需要处理请手动编辑 patch。',
  skipUnknownPackage:
    '[dsh-safe] 跳过 {id}：无法确定它属于哪个包，不能排除是 dsh 的核心依赖，因此不自动禁用；失败原因: {reason}。',
  nothingFound: '[dsh-safe] 报错里没有识别出可自动隔离的已挂载插件，原样透传。',
  envFailure: '[dsh-safe] 环境类失败，未隔离任何插件：{label} — {reason}',
  envFailureHint:
    '[dsh-safe] 请先解决环境问题再重启（端口被占通常是已有另一个 dsh 实例在跑）；本次涉及的插件全部保持启用。',
  ...
  updateAlreadyLatest: '[dsh-safe] latest 通道已是最新，无需更新（dsh {dsh} / dsh-safe {self}）。',
  updateChannelHint: '[dsh-safe] {tag} 通道已有 {version}（当前 {current}）；跟进: dsh-safe update --to {tag}',
  updateCheckWouldRun: '[dsh-safe] --check：本次不做任何改动；去掉 --check 将运行 {command}',
  ...
  updateToInvalid: '[dsh-safe] --to 需要合法的版本号（如 1.2.3 / 1.2.3-rc.1）或 dist-tag（如 next）。',
...
Usage:
  ...
  dsh-safe -u [<dsh args…>]         upgrade dsh first (skip if latest), then boot;
                                    with no args at all it only upgrades, without booting
  ...
  dsh-safe update [-y] [--to <ver|tag>] [--check] [--self] [--no-restore] [--no-verify] [--pm npm|pnpm]
                                    upgrade dsh and dsh-safe itself, auto-restore quarantined plugins;
                                    --check only reports whether/what would be upgraded, without installing or booting
...
Wrapper-mode options (must come before the profile / subcommand):
  ...
  --allow-first-party      allow removing an official bundle during duplicate dedupe (quarantine never touches core dependencies)
...
Notes:
  ...
  - Only startup-phase failures are quarantined (module resolution / apply throw /
    ...
    fail-loud policy.
  - Environment failures (EADDRINUSE port already in use / permissions / network, …)
    are never quarantined and never write any file; the exit code is passed through
    with an explanation — an environment problem makes healthy plugins fail too, so
    any quarantine decision from it would be a misdiagnosis.
  - Entries dsh itself depends on (official plugins, rows mounted by an official
    bundle, the loader mechanism, the web UI host entry webserver) are never
    auto-disabled and no flag opens them up; rows whose package cannot be
    determined are not quarantined either. Edit the patch manually if needed.
  - update/-u checks both dsh and dsh-safe itself; --self updates dsh-safe only.
  - The npm "latest" channel is the only one followed. When channels other than
    latest (next / alpha / beta, …) have something newer, one hint line per channel
    is printed, ascending by version (never installed automatically); to follow one:
    dsh-safe update --to <tag> (--to also accepts a dist-tag). update / --check and
    the -u <dsh args> boot path all report it every time; the boot path also prints
    a status line ("already up to date" / "updating").
  - A new-version notice for dsh-safe is shown at most once a day on boot;
    DSH_SAFE_NO_UPDATE_CHECK=1 disables the boot-time update report (status +
    channel hints) and that daily notice (explicit update / --check are not
    affected by that switch).
...
`,
  ...
  skipCore:
    '[dsh-safe] skipping core dependency {id} (a plugin dsh itself depends on, a row mounted by an official bundle, or the loader mechanism; never auto-disabled under any circumstance); failure: {reason}. Edit the patch manually if you need to act on it.',
  skipUnknownPackage:
    '[dsh-safe] skipping {id}: its package could not be determined, so it cannot be ruled out as a dsh core dependency; failure: {reason}.',
  nothingFound:
    '[dsh-safe] no mounted plugin could be identified as auto-quarantinable from the error; exit code passed through.',
  envFailure: '[dsh-safe] environment failure; nothing was quarantined: {label} — {reason}',
  envFailureHint:
    '[dsh-safe] fix the environment problem and restart (a port already in use usually means another dsh instance is running); every plugin involved was left enabled.',
  ...
  updateAlreadyLatest: '[dsh-safe] already up to date on the latest channel (dsh {dsh} / dsh-safe {self}); nothing to update.',
  updateChannelHint: '[dsh-safe] the {tag} channel has {version} (current {current}); to follow it: dsh-safe update --to {tag}',
  updateCheckWouldRun: '[dsh-safe] --check: nothing was changed; without --check this would run: {command}',
  ...
  updateToInvalid: '[dsh-safe] --to requires a valid version (e.g. 1.2.3 or 1.2.3-rc.1) or a dist-tag (e.g. next).',
