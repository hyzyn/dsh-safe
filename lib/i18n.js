...
说明:
  ...
  - webserver（web UI 入口）与加载器机制层（include / cordis:*）是保留条目，永不
    自动禁用，--allow-first-party 也放不开；需要处理请手动编辑 patch。
...
`,
  ...
  skipReserved:
    '[dsh-safe] 保留条目 {id} 永不自动禁用（禁掉它会让 web UI 或加载机制静默失效）；失败原因: {reason}。需要处理请手动编辑 patch。',
...
Notes:
  ...
  - webserver (the web UI host entry) and loader-mechanism rows (include / cordis:*)
    are reserved: never auto-disabled, and --allow-first-party does not open them up;
    edit the patch manually if you need to act on them.
...
`,
  ...
  skipReserved:
    '[dsh-safe] reserved entry {id} is never auto-disabled (disabling it would silently break the web UI or the loader mechanism); failure: {reason}. Edit the patch manually if you need to act on it.',
  nothingFound:
