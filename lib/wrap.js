...
import { dedupeMountSources, isFirstParty, isReservedEntry } from './dedupe.js'
...
export async function runWrapped(options) {
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    for (const hit of hits) {
      ...
      // 保留条目优先于一切：不依赖包名解析，也放不开（见 dedupe.js 的说明）
      if (isReservedEntry(hit.id, hit.name)) reserved.push(hit)
      else if (isFirstParty(hit.name) && !allowFirstParty) firstParty.push(hit)
      else if (isExcluded(hit)) log(t('excludedByList', { label: hit.name ?? hit.id }))
      else quarantinable.push(hit)
    }
    for (const hit of reserved) {
      log(t('skipReserved', { id: hit.id, reason: summarizeLine(hit.line) }))
    }
    for (const hit of firstParty) {
      log(t('skipFirstParty', { name: hit.name, id: hit.id }))
    }
    if (!quarantinable.length) {
      if (!firstParty.length && !reserved.length) {
        log(t('nothingFound'))
        log(t('explainHint'))
      }
      return code
