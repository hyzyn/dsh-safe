...
const RESERVED_IDS = new Set([
  // web UI 的宿主入口。它失败常常是端口被另一个 dsh 实例占用（环境问题），
  // 禁用它会让 UI 静默消失而真正的原因仍在；它本身也是官方包。
  'webserver',
  // patch 合成链路的机制层包装（报错里 id=include、name=cordis:include）。
  // 禁用它等于拆掉 bundle 的挂载机制本身。
  'include',
])
...
const RESERVED_NAME_PREFIXES = [
  'cordis:', // 机制层命名空间
  '@deepseek-ai/dsh-host-webserver', // web UI 宿主包：按包名再兜一层
]

/**
 * 保留条目：永不自动禁用，且**不可被任何旗标放开**（`--allow-first-party` 对它无效，
 * 因为它不是"第一方保护"的下级，而是独立的结构性兜底）。
 *
 * 为什么需要它：第一方保护依赖包名可解析，而真实 profile 里包名可能只存在于 dsh 的
 * 报错文本、甚至只剩 entry id（外层栈命中）。任何"名字解析失败"都会让保护静默失效——
 * 实测中就发生过官方 webserver 被禁用、台账记成 name: null。保留条目不依赖名字解析，
 * 因此不受这条链路影响。
 *
 * 确实需要禁用这些条目时请手动编辑 patch——这是刻意的，不提供旗标。
 */
export const isReservedEntry = (id, name) =>
  RESERVED_IDS.has(id) ||
  (typeof name === 'string' && RESERVED_NAME_PREFIXES.some((prefix) => name.startsWith(prefix)))

/** 该 id 是否由官方 bundle 挂载（patch 行的来源 bundle 属于官方命名空间）。 */
export const isMountedByOfficialBundle = (known, id) =>
  (known?.rows ?? []).some((row) => row.id === id && isFirstParty(row.source))

/**
 * dsh 本身的核心依赖：**任何情况下都不自动禁用**，且不受任何旗标影响。
 *
 * 三条互相独立的信号，任一成立即为核心。刻意不把保证押在单一来源上——这次实测事故
 * 就是"保护只依赖包名可解析"这一条链路断掉导致的（官方 webserver 被禁用、台账记成
 * name: null）：
 *   1) 保留条目：webserver / include / cordis:*（见 isReservedEntry）
 *   2) 由官方 bundle 挂载的行：结构信号，不需要知道包名
 *   3) 包名属于官方命名空间 @deepseek-ai/*
 */
export const isCoreEntry = (hit, known) =>
  isReservedEntry(hit.id, hit.name) || isMountedByOfficialBundle(known, hit.id) || isFirstParty(hit.name)

/**
 * 包名无法确定：既不能确认它是第三方，就不能排除它是核心依赖——按核心对待（fail closed）。
 * 隔离是持久写入，宁可漏隔离也不能误伤；需要处理时手动编辑 patch。
 */
export const hasUnknownPackage = (hit) => typeof hit.name !== 'string' || hit.name === ''

/**
 * 该 id 的 bundle 挂载来源（profile/home 层是 override，不算挂载来源）。
 * internal 行（dsh 安装目录里的官方 bundle）不算：它们的加入只是为了包名/来源解析，
 * 让去重与 repair 的重复判定保持原样——否则"修好 bundle 收集"会顺带改变会对 manifest
 * 动手的路径，那是另一个需要单独决定的高危面。
 */
export function bundleMountSources(known, id) {
  return [
    ...new Set(
      known.rows
        .filter((r) => r.id === id && r.source !== 'profile' && r.source !== 'home' && !r.internal)
        .map((r) => r.source),
    ),
  ]
}
