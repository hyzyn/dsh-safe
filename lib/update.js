...
export function resolveDshPackage() {
  ...
  return resolvePackageFromDir(dirname(real))
}

/**
 * 解析正在运行的 dsh-safe 自身（argv[1] 的 realpath 向上找 package.json）。
 * 通过 npm/pnpm 全局 bin symlink 启动时，realpath 落在安装目录内。
 * @returns {{ name: string, version: string, pkgDir: string } | null}
 */
...
function fetchLatestVersion(name) {
  const bin = whichCmd('npm')
  const { status, stdout } = spawnSync(bin ?? 'npm', ['view', name, 'version'], { encoding: 'utf8' })
