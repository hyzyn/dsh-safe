...
async function updateAndMaybeBoot(opts, bootArgs, { boot, verify = verifyParser } = {}) {
  ...
  // latest 之外的通道差距（next 比"将装到的版本"更新时给一条指引）。只提示，绝不
  // 自动安装——dsh-safe 不替用户决定上未发布通道。显式 update / --check 每次都提示；
  // -u <dsh 参数> 的日常启动路径受每日一次闸（及 DSH_SAFE_NO_UPDATE_CHECK）约束。
  const notifyChannelGap = () => {
    if (!dshTags?.next) return
    const target = plans.find((p) => p.pkg === dshPkg)?.target ?? dshPkg.version
    if (!isNewerVersion(dshTags.next, target)) return
    const bootNotice = Boolean(bootArgs) && !opts.check
    if (bootNotice && (noUpdateCheck() || !takeDailySlot('channelNotifyAt'))) return
    err(t('updateNextHint', { next: dshTags.next, current: target }))
  }
