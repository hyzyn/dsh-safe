...
/**
 * latest 之外的通道差距：从 dist-tags 里挑出比"将装到的版本"更新的通道。
 * 通道名不写死——上游的未发布通道不止 next（还有 alpha/beta/canary…），
 * 只排除 latest；多个通道都更新时取版本最高的那个（版本相同则按通道名字典序，
 * 与 registry 返回的键顺序无关）。比较用 isNewerVersion，所以同一核心版本的
 * 预发布（1.2.3-alpha.1 对 1.2.3）不会被误报成"有更新"。
 ...
 */
export function channelGap(tags, target) {
  if (!tags || !target) return null
  let best = null
  for (const [tag, version] of Object.entries(tags)) {
    if (tag === 'latest') continue
    if (!isNewerVersion(version, target)) continue
    if (!best || isNewerVersion(version, best.version) || (version === best.version && tag < best.tag)) {
      best = { tag, version }
    }
  }
  return best
...
async function updateAndMaybeBoot(opts, bootArgs, { boot, verify = verifyParser } = {}) {
  ...
  // 只提示，绝不自动安装——dsh-safe 不替用户决定上未发布通道。显式 update / --check
  // 每次都提示；-u <dsh 参数> 的日常启动路径受每日一次闸（及 DSH_SAFE_NO_UPDATE_CHECK）约束。
  const notifyChannelGap = () => {
    if (!dshPkg) return
    const target = plans.find((p) => p.pkg === dshPkg)?.target ?? dshPkg.version
    const gap = channelGap(dshTags, target)
    if (!gap) return
    const bootNotice = Boolean(bootArgs) && !opts.check
    if (bootNotice && (noUpdateCheck() || !takeDailySlot('channelNotifyAt'))) return
